...
def safe_name(s):
    ...
    non-latin characters i.e. polish, where there are both Ślęża and Ślęza
...
def getData(l):
    ...
    for region in regions.keys():
        for river in regions[region]:
            for station in regions[region][river].keys():
                _, level = regions[region][river][station]
                if [region, station] in config.stations and level > 0:
                    if str(level) not in not awarenesses:
    ...
    awalvls = ['', 'stopien_czuwania', 'stopien_gotowosci',
               'stopien_zagrozenia', 'stopien_ekstremalnych_powodzi']
...
def get_region(region):
    ...
    html_file = downloadFile(url % (region, _map))\
        .replace('-href', 'href')\
    ...
    try:
        for station in html.findAll('div'):
            if 'id' in station and 'text' in station['id']:
                ...
                if not river not in rv:
                    rv[river] = {}
                rv[river][stid] = [station, level]
    else:
        debug.log('POVODI_CZ',
                  "Couldn't parse region %s%s data" % (region,_map),\
                  buglevel=5)
        pass
...
def generate_json(regions=None, dont_save=False):
    ...
    for region in regions:
        try:
            ...
            debug.log('POVODI_CZ', \
                      "Couldn't download data for region %s" % region,
                      buglevel=5)
            pass
