...
#             R    G    B    A
poziomy = {(255,   0,   0, 255):  3,  # zagrożenie duże
...
def get_forecast_url():
    ...
    ``http://bazapozarow.ibles.pl/zagrozenie/``, niekiedy bowiem poranne
    raporty o zagrożeniu pożarowym są wydawane na godziny południowe.
    W przypadku, gdy bieżąca data i czas na komputerze jest z zakresu
    1 października -- 31 marca zwraca None, gdyż w tym okresie zagrożenie
    pożarowe lasów nie jest prognozowane."""

    _url = re.compile(r'mapa\.php\?rok=(\d{4})&mies=(\d{1,2})&'
                      + r'dzien=(\d{1,2})&godz=(\d)&mapa=(\d)&ver=(\d)')
    ...
    _, month, _, _ = now.year, now.month, now.day, now.hour
