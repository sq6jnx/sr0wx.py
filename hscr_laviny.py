...
def getAwareness(region):
    ...
    r = u'id\=(\d{4,}).{10,}Lavinov.{5,}' + region
    _id = re.compile(r)
...
def getData(l):
    ...
    level, tendention, _, isActual, infoDT = getAwareness(config.region)
    ...
    data["data"] = " ".join((data["data"],
                             lang.hscr_region[config.region[0]],
                             lang.avalancheLevel[level]))

    if config.giveTendention == 1:
        data["data"] = " ".join((data["data"],
                                 lang.hscr_tendention[tendention]))

    if not isActual:
        data["data"] = " ".join((data["data"],
                                 lang.info_at,
                                 lang.readISODate(infoDT.isoformat(' '))))

    data["data"] = lang.removeDiacritics(lang.hscr_welcome
                                         + ' _ '
                                         + data["data"])
