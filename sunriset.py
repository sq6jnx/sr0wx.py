...
def getLocalTimeFromISO(isoDT, timeZone=config.timeZone):
    # This tiny function is used to convert from UTC to local time. Code
    # inspired by ``pytz`` manual, http://pytz.sourceforge.net/#example-usage
    # TODO: possible code duplication and misuse, see ISODT functions
    # in lang.
    y, m, d, hh, mm, ss = (int(isoDT[0:4]), int(isoDT[5:7]),
                           int(isoDT[8:10]), int(isoDT[11:13]),
                           int(isoDT[14:16]), int(isoDT[17:19]))
...
def float2datetime(time, date=datetime.datetime.utcnow()):
    ...
    time = (time - mm)  * 60
    ss = int(time)
    time = (time - ss) * (1000 * 1000)
    ms = int(time)

    return datetime.datetime(date.year, date.month, date.day, hh, mm, ss, ms,
...
def getData(l):
    ...
    sun = lib.Sun.Sun()
    sunrise, sunset = sun.sunRiseSet(now.year, now.month, now.day,
                                     config.location[0],
                                     config.location[1])
