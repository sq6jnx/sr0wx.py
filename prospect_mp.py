...
przekroczenie = re.compile('(Delta)(?:(?:.{1,}?)Przekroczony\ stan\ (ostrzegawczy|alarmowy))?')
...
def pobierzOstrzezenia(domena, stacja):
    ...
    try:
        ...
        if wynik[0] == ('Delta', ''):
            ...
            debug.log('PROSPECT-MP',
                      u'Regex nie zwrócił oczekiwanych danych',
                      buglevel=5)
            return None
    except:
        debug.log('PROSPECT-MP',
...
def generuj_json(nie_zapisuj=False):
    ...
    for w in config.wodowskazy:
        try:
            ...
            raise
            debug.log('PROSPECT-MP',
                      u'Pobieranie danych zakończyło się błędem',
                      buglevel=5)
            pass
